...
class awp_checkout_otp
{
    ...
    public function enqueue_scripts()
    {
        ...
        wp_localize_script('awp-checkout-js', 'otpAjax', array(
    ...
    public function settings_page_html()
    {
        ...
        include plugin_dir_path(__FILE__) . 'admin/awp-checkout-admin.php';
    }
    ...
    public function admin_enqueue_scripts($hook)
    {
        ...
        if ('admin.php' === $pagenow && isset($_GET['page']) && 'awp-checkout-otp' === $_GET['page']) {
            ...
            wp_localize_script('awp-admin-js', 'ajax_object', array(
    ...
    public function verify_phone_number_ajax_handler()
{
    ...
    $phone_number = sanitize_text_field($_POST['phone_number']);
    if (empty($phone_number)) {
        wp_send_json_error(array('status' => 'error', 'message' => esc_html__('Phone number is missing.', 'awp')));
        wp_die();
    }
    ...
    if (in_array($phone_number, $blocked_numbers)) {
        wp_send_json_error(array('status' => 'blocked', 'message' => esc_html__('This phone number is blocked.', 'awp')));
        wp_die();
    }
    ...
    if (is_user_logged_in()) {
        ...
        if ($phone_number === $user_phone_number) {
            if ($phone_verified) {
                wp_send_json_success(array('status' => 'verified', 'message' => esc_html__('Phone number matches user account and is verified.', 'awp')));
            } else {
                wp_send_json_error(array('status' => 'not_verified', 'message' => esc_html__('Phone number matches user account but is not verified.', 'awp')));
            }
            wp_die();
        }
    }
    ...
    if (!empty($user_query->get_results())) {
        wp_send_json_error(array('status' => 'registered', 'message' => esc_html__('This phone number is already registered. Please use a different number.', 'awp')));
        wp_die();
    }

    wp_send_json_success(array('status' => 'not_registered', 'message' => esc_html__('Phone number is not registered.', 'awp')));
}
